...
if __name__ == "__main__":
    ...
    def prepare_for_pairs(df):
        df_anomaly = df.groupby('labels').get_group(dataset.int2label['anomaly']).loc[:, 'txt']
        df_normal = df.groupby('labels').get_group(dataset.int2label['normal']).loc[:, 'txt']
    ...
    df_normal_train, df_anomaly_train = prepare_for_pairs(df_train)
    ...
    # df_test_sample_normal = df_test.groupby('labels').get_group(dataset.int2label['normal']).head(3).loc[:, ['txt']] # 3 representant
    # print(df_test_sample_normal.loc[:, ['txt']])
    ...
    df_test_sample_normal = df_test.groupby('labels').get_group(dataset.int2label['normal']).head(
    ...
    df_test_expand = pd.concat([df_test] * NB_REFERENCE_NORMAL)  # Keep the index intact
